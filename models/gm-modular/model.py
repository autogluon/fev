...
SNAIVE (seasonal naive), AVG3 (trailing average), SIDX (seasonal index × trend),
...
class GmModularModel(fev.ForecastingModel):
    ...
    def _predict_window(
        ...
        ts_col = window.timestamp_column
        ...
        for sid, grp in past_df.groupby(id_col, sort=False):
...
# Candidate model names (used internally)
_SNAIVE = "SNAIVE"
_AVG3 = "AVG3"
_SIDX = "SIDX"
_HW = "HW"
...
def _gm_forecast(train: list[float], horizon: int) -> list[float]:
    ...
    2. Validate each candidate on a held-out window
    3. Pick champion by WAPE
    4. Retrain champion on full train, forecast horizon steps
    """
    # Clean: filter out NaN/inf
    clean = [0.0 if math.isnan(v) or math.isinf(v) else v for v in train]
    if not clean or all(v == 0 for v in clean):
        return [0.0] * horizon

    seg = _segment(clean)
    # Validation window: use up to last horizon*2 points (or 1/3 of series)
    ...
    if not train_inner or len(train_inner) < 2:
        base = statistics.mean(clean[-3:]) if clean else 0.0
        return [base] * horizon
    ...
    # Retrain champion on full clean data
...
def _sanitize(values: list[float]) -> list[float]:
    """Replace NaN/inf with 0.0 and return clean list."""
...
def _safe_pstdev(values: list[float]) -> float:
    """Population stdev with NaN/inf guards."""
    clean = _sanitize(values)
    if len(clean) < 2:
        return 0.0
...
def _segment(y: list[float]) -> str:
    """Classify a series: STABLE, SEASONAL, LUMPY, VOLATILE, or NEW."""
    clean = _sanitize(y)
...
def _seasonality_strength(y: list[float], period: int = 12) -> float:
    """Dispersion of monthly seasonal indices on the detrended series."""
    clean = _sanitize(y)
    if sum(clean) == 0:
        return 0.0
    n = len(clean)
...
def _f_snaive(train: list[float], horizon: int) -> list[float]:
    """Seasonal naive: same value from 12 steps ago (or trailing avg fallback)."""
...
def _f_avg3(train: list[float], horizon: int) -> list[float]:
    """Trailing 3-period average, held constant."""
    if not train:
        return [0.0] * horizon
...
def _f_sidx(train: list[float], horizon: int, period: int = 12) -> list[float]:
    """Seasonal index × linear trend (descasonalise, fit trend, reseasonalise)."""
...
def _f_hw(train: list[float], horizon: int, period: int = 12) -> list[float]:
    """Holt-Winters additive with grid search over alpha, gamma."""
...
def _estimate_quantile_widths(
    ...
    """Estimate per-step std dev for quantile bands via simple residual scaling."""
    ...
    # Simple deseasonalise-difference residual std
...
def _normal_quantile(q: float) -> float:
    ...
    # Abramowitz & Stegun approximation (Hastings), accurate to ~1e-4
    if q <= 0 or q >= 1:
        return 0.0
